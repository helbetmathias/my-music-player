...
export default function App() {
  ...
  const fetchLyrics = async (artist, title) => {
    ...
    const cleanArtist = artist.split(/feat|ft\.|,|&/i)[0].replace(/["']/g, "").trim();
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e) => {
      ...
      switch (e.code) {
        ...
        case 'ArrowRight': if(e.ctrlKey) playNext(); else if(audioRef.current) audioRef.current.currentTime = Math.min(audioRef.current.duration, audioRef.current.currentTime + 5); break;
        case 'ArrowLeft': if(e.ctrlKey) playPrev(); else if(audioRef.current) audioRef.current.currentTime = Math.max(0, audioRef.current.currentTime - 5); break;
  ...
  const clearPlaylist = () => {
    ...
    setPlaylist([]); setCurrentTrackIndex(-1); setIsPlaying(false); setDuration(0); setCurrentTime(0); setActiveCoverArt(null); currentTrackIdRef.current = null; setThemeColor("rgb(99, 102, 241)"); setHistory([]);
  };
          ...
          <div className="z-10 flex flex-col items-center max-w-2xl w-full text-center space-y-6 md:space-y-8 h-full justify-center">
            ...
            {showLyrics ? (
               <div className="w-full h-96 md:h-[500px] bg-black/40 rounded-2xl border border-slate-800 backdrop-blur-sm overflow-hidden relative flex flex-col animate-in fade-in zoom-in duration-300">
                  ...
                  <div className="flex-1 overflow-y-auto p-6 space-y-6 scrollbar-custom text-center" ref={lyricsContainerRef}>
                     {lyricsLoading ? (
